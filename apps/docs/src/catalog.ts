...
export const componentDocs: ComponentDoc[] = [
  ...
  doc('KvTooltip', 'Overlays', 'Delayed hover/focus description forwarded directly to one required HTML trigger element.', [{ name: 'text', type: 'string', description: 'Tooltip content.' }, { name: 'delay', type: 'number', default: '350', description: 'Open delay in milliseconds.' }], ['Focus — show', 'Escape / Blur — hide'], `<KvTooltip text="Transmission details"><button type="button">Inspect</button></KvTooltip>`),
  ...
  doc('KvAlert', 'Feedback', 'Inline visual status with explicit, opt-in live-region semantics and optional dismissal.', [{ name: 'status', type: 'KvStatus', default: "'neutral'", description: 'Visual status color only.' }, { name: 'announce', type: "'off' | 'polite' | 'assertive'", default: "'off'", description: 'Opt-in announcement priority. Polite maps to status; assertive maps to alert.' }]),
